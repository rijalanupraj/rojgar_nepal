...
class CompanyDashboard(tk.Frame):
    def __init__(self, master):
        master.root.title('Dashboard')
        self.mastery = master
        self.dbconnect = DbConnection()

        self.company_name_entry = StringVar()
        self.company_username_entry = StringVar()
        self.company_phonenumber_entry = StringVar()
        self.company_address_entry = StringVar()
        self.company_confirm_password_entry = StringVar()
        self.company_password_entry = StringVar()
        ...
        self.name_entry = tk.Entry(profile_frame, state='readonly', textvariable=self.company_name_entry,
        ...
        self.username_entry = tk.Entry(profile_frame, state='readonly', textvariable=self.company_username_entry,
                                       width=entry_widget_width, font=entry_widget_font)
        ...
        self.phonenumber_entry = tk.Entry(profile_frame, state='readonly', textvariable=self.company_phonenumber_entry,
                                          width=entry_widget_width, font=entry_widget_font)
        ...
        self.address_entry = tk.Entry(profile_frame, state='readonly', textvariable=self.company_address_entry,
                                      width=entry_widget_width, font=entry_widget_font)
        self.address_entry.grid(row=6, column=1, pady=(0, 4), ipady=entry_widget_ipday)

        self.password_label = tk.Label(profile_frame, text="New Password", font=label_widget_font)
        self.password_entry = tk.Entry(profile_frame, textvariable=self.company_password_entry, show='*',
                                       width=entry_widget_width, font=entry_widget_font)

        self.confirm_password_label = tk.Label(profile_frame, text="Password", font=label_widget_font)
        self.confirm_password_entry = tk.Entry(profile_frame, textvariable=self.company_confirm_password_entry,
                                               show='*', width=entry_widget_width, font=entry_widget_font)
        ...
        self.company_name_entry.set(self.company_ref.get_name())
        self.company_username_entry.set(self.company_ref.get_username())
        self.company_phonenumber_entry.set(self.company_ref.get_phone())
        self.company_address_entry.set(self.company_ref.get_address())
    ...
    def update_button_clicked(self):
        ...
        name_current_user = self.company_name_entry.get()
        username_current_user = self.company_username_entry.get().lower()
        address_current_user = self.company_address_entry.get()
        password_current_user = self.company_password_entry.get()
        confirmation_password_current_user = self.company_confirm_password_entry.get()
        phonenumber_current_user = self.company_phonenumber_entry.get()
        ...
        system_username_list.remove(self.company_ref.get_username())
        ...
        # Checking if the password and confirm password is same or not.
            ...
            company_ref = Company(name_current_user, username_current_user, phonenumber_current_user,
                                  address_current_user, password_current_user)
            query = "UPDATE company set `name`=%s,`username`=%s,`phone`=%s,`address`=%s,`password`=%s where id=%s;"
            values = (
                company_ref.get_name(), company_ref.get_username(), company_ref.get_phone(), company_ref.get_address(),
                company_ref.get_password(), self.company_ref.get_id())
    ...
    def delete_account(self):
        ...
        if result:
            ...
            values = (self.company_ref.get_id(),)
            self.dbconnect.insert(query, values)
            self.mastery.switch_frame('loginWindow')
